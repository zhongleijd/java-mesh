...
public class PerfTestRunnable implements ControllerConstants {
    ...
    public static Set<ITestLifeCycleListener> allTestLifeCycleListeners = new HashSet<>();
    ...
    private boolean isScheduledNow(PerfTest test) {
        ...
        Date scheduleDate = (Date) defaultIfNull(test.getScheduledTime(), current);
        Date scheduledDate = DateUtils.truncate(scheduleDate, Calendar.SECOND);
    ...
    protected boolean hasSelectFreeAgents(PerfTest test) {
        if (StringUtils.isBlank(test.getAgentIds())) {
            return true;
        }
        Set<AgentIdentity> allFreeApprovedAgentsForUser = agentManager.getAllFreeApprovedAgentsForUser(test.getCreatedUser());
        List<String> freeAgentNameList = new ArrayList<>();
        for (AgentIdentity agentIdentity : allFreeApprovedAgentsForUser) {
            String name = agentIdentity.getName();
            freeAgentNameList.add(name);
        }
        List<String> selectAgentNameList = perfTestTaskService.getSelectAgentNameList(test);
        boolean b = freeAgentNameList.containsAll(selectAgentNameList);
        if (b) {
            return true;
        }

        perfTestService.markProgress(test, "The test is tried to execute but selected agents is not enough."
            + "\n- Current free agents is : " + freeAgentNameList + "  / Requested : " + selectAgentNameList + "\n");
        return false;
    ...
    void distributeFileOn(final PerfTest perfTest, SingleConsole singleConsole) {
        ...
        File distributionPath = perfTestService.getDistributionPath(perfTest);
        singleConsole.distributeFiles(distributionPath, listener, isSafeDistPerfTest(perfTest));
    ...
    void startAgentsOn(PerfTest perfTest, GrinderProperties grinderProperties, SingleConsole singleConsole) {
        perfTestService.markStatusAndProgress(perfTest, START_AGENTS, getSafe(perfTest.getAgentCount())
            + " agents are starting.");
        if (StringUtils.isBlank(perfTest.getAgentIds())) {
            agentManager.runAgent(perfTest.getCreatedUser(), singleConsole, grinderProperties, getSafe(perfTest.getAgentCount()));
        } else {
            agentManager.runSelectAgent(perfTest, singleConsole, grinderProperties, getSafe(perfTest.getAgentCount()));
        }
    ...
    public void notifyFinish(PerfTest perfTest, StopReason reason) {
        ...
        for (ITestLifeCycleListener listener : allTestLifeCycleListeners) {
            listener.finish(perfTest, reason.name(), perfTestService, config.getVersion());
        }
    }

    public void notifyStart(PerfTest perfTest) {
        for (OnTestLifeCycleRunnable run : pluginManager.getEnabledModulesByClass(OnTestLifeCycleRunnable.class)) {
            run.start(perfTest, perfTestService, config.getVersion());
        }
        for (ITestLifeCycleListener listener : allTestLifeCycleListeners) {
            listener.start(perfTest, perfTestService, config.getVersion());
        }
    }
    ...
    // NEW
    private void saveReport(PerfTest perfTest) {
        Long id = perfTest.getId();
        PerfTestReport perfTestReport = perfTestReportService.saveApiBasicReport(id, 600);
        perfTestReportService.savePerfGraph(id, perfTestReport, "TPS,Errors,Mean_Test_Time_(ms),Mean_time_to_first_byte,User_defined,Vuser", false, 600);
    }
