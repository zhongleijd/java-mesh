...
//	private String svnHome = "svn://100.95.184.191/admin";
	...
	/**
	 * @param user the user
	 * @return user svn repository path
	 */
	public String getUserSvnRepoPath(User user) {
		return home.getUserSvnRepoPath(user);
	}
	...
	/**
	 * NEW 获取SVN仓库根目录下所有文件夹
	 * @param repository
	 * @return
	 * @throws SVNException
	 */
	public List<String> getRootEntries(SVNRepository repository) throws SVNException {
		String path = "";
		...
		while (iterator.hasNext()) {
			...
			this.entryList.add("/" + entry.getName());
		}
		return rootEntries;
	}

	/**
	 * NEW 获取SVN仓库所有文件路径,存入this.entryList
	 *
	 * @param repository
	 * @param path
	 * @throws SVNException
	 */
	public void getEntries(SVNRepository repository, String path)
		throws SVNException {
		...
		while (iterator.hasNext()) {
			SVNDirEntry entry = (SVNDirEntry) iterator.next();
			this.entryList.add("/" + (path.equals("") ? "" : path + "/") + entry.getName());
			/*
			 * Checking up if the entry is a directory.
			 */
			if (entry.getKind() == SVNNodeKind.DIR) {
				getEntries(repository, (path.equals("")) ? entry.getName()
					: path + "/" + entry.getName());
			}
		}
	}
	 ...
	 * @return
	 */
	public boolean checkIfDirExist(String fullPath) {
		boolean exist = false;
		String tempPath = fullPath;
		if (!tempPath.startsWith("/"))
			tempPath = "/" + tempPath;
		if (this.entryList.contains(tempPath))
			return true;
		return false;
	}

	/**
	 * 在SVN仓库根目录下创建新的用户文件夹，文件夹名称为user.name
	 * @param user
	 * @return
	 * @throws SVNException
	 */
	public void createUserRepoDir(User user) throws SVNException {
		SVNClientManager svnClientManager = null;
		ISVNEditor editor = null;
		svnClientManager = getSVNClientManager();
		SVNRepository repo = svnClientManager.createRepository(SVNURL.parseURIEncoded(getSvnRepoPath()), true);

		try {
			editor = repo.getCommitEditor("", null, true, null);
			editor.openRoot(-1);

			editor.addDir(user.getUserName(), null, -1);
		} catch (Exception e) {
	 ...
	 * @return
	 */
	public boolean checkIfUserSvnDirExist(User user) throws SVNException {
		SVNClientManager svnClientManager = null;
		svnClientManager = getSVNClientManager();
		SVNRepository repo = svnClientManager.createRepository(SVNURL.parseURIEncoded(getSvnRepoPath()), true);

		List<String> rootEntries = this.getRootEntries(repo);

		if (rootEntries.contains(this.getUserSvnRepoPath(user)))
			return true;
		return false;
	}

	/**
	 * Save fileEntry on the {FileEntry.getPath()} location.
	 *
	 * @param user      the user
