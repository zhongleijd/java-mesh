...
public abstract class ScriptHandler implements ControllerConstants {
     ...
     */
    public List<FileEntry> getLibAndResourceEntries(User user, FileEntry scriptEntry, long revision) throws IOException {
        String path = FilenameUtils.getPath(scriptEntry.getPath());
        List<FileEntry> fileList = newArrayList();
        for (FileEntry eachFileEntry : nfsFileEntryService.getUserScriptAllFiles(user, path + "lib/")) {
        ...
        for (FileEntry eachFileEntry : nfsFileEntryService.getUserScriptAllFiles(user, path + "resources/")) {
            FileType fileType = eachFileEntry.getFileType();
    ...
    public NfsFileEntryService getNfsFileEntryService() {
        return nfsFileEntryService;
    }
