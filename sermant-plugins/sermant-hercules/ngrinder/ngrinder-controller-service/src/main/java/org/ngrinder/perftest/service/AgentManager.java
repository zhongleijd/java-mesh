...
public class AgentManager implements ControllerConstants, AgentDownloadRequestListener {
	...
	public synchronized void runSelectAgent(final PerfTest perfTest, final SingleConsole singleConsole,
											final GrinderProperties grinderProperties, final Integer agentCount) {
	...
	public Set<AgentIdentity> selectAgentByUser(User user, Set<AgentIdentity> allFreeAgents, int agentCount,PerfTest perfTest) {
		Set<AgentIdentity> userAgent = new HashSet<AgentIdentity>();
		List<String> selectAgentNameList = perfTestTaskService.getSelectAgentNameList(perfTest);
		for (AgentIdentity each : allFreeAgents) {
			String region = ((AgentControllerIdentityImplementation) each).getRegion();
			if (StringUtils.endsWith(region, "owned_" + user.getUserId()) && selectAgentNameList.contains(each.getName())) {
				userAgent.add(each);
				if (userAgent.size() == agentCount) {
					return userAgent;
				}
		...
		for (AgentIdentity each : allFreeAgents) {
			String region = ((AgentControllerIdentityImplementation) each).getRegion();
			if (!StringUtils.contains(region, "owned_") && selectAgentNameList.contains(each.getName())) {
				userAgent.add(each);
				if (userAgent.size() == agentCount) {
					return userAgent;
				}
