...
public class FileEntryController extends BaseController {
	...
	@RestAPI
	@ResponseBody
	@RequestMapping(value = "/api/new/**", params = "type=folder", method = RequestMethod.POST)
	public ResponseEntity<FileEntry> addFolderApi(User user, @RemainedPath String path,
	...
	@RestAPI
	@ResponseBody
	@RequestMapping(value = "/api/new/**", params = "type=script", method = RequestMethod.POST)
	public HttpEntity<String> createFormApi(User user, @RemainedPath String path,
							 @RequestParam(value = "testUrl", required = false) String testUrl,
							 @RequestParam("fileName") String fileName,
							 @RequestParam(value = "scriptType", required = false) String scriptType,
							 @RequestParam(value = "createLibAndResource", defaultValue = "false") boolean createLibAndResources,
	...
	public String deleteApi(User user, @RemainedPath String path, @RequestParam("filesString") String filesString) {
		String[] files = filesString.split(",");
		fileEntryService.delete(user, path, files);
		Map<String, Object> rtnMap = new HashMap<String, Object>(1);
		rtnMap.put(JSON_SUCCESS, true);
	...
	public HttpEntity<JSONObject> uploadAPI(User user, @RemainedPath String path,
	...
	public void downloadApi(User user, @RemainedPath String path, HttpServletResponse response) {
		FileEntry fileEntry = fileEntryService.getOne(user, path);
		if (fileEntry == null) {
			LOG.error("{} requested to download not existing file entity {}", user.getUserId(), path);
			return;
	...
	public HttpEntity<JSONObject> uploadForAPI(User user, @RemainedPath String path,
	                                       @RequestParam("description") String description,
	                                       @RequestParam("uploadFile") MultipartFile file) throws IOException {
		upload(user, path, description, file);
		return successJsonHttpEntity();
	...
	public HttpEntity<JSONObject> deleteOne(User user, @RemainedPath String path) {
