...
import org.ngrinder.script.handler.FlowMavenProjectScriptHandler;
...
//		fileEntryCache = cacheManager.getCache("file_entries");
	...
	public FileEntry prepareNewEntryByKey(User user, String path, String fileName, String name, String url,
										  ScriptHandler scriptHandler, boolean libAndResource, String options) {
		scriptHandler.prepareScriptEnv(user, path, fileName, name, url, libAndResource,
			loadTemplate(user, scriptHandler, url, name, options));
		return null;
	}
	...
	public void prepareNewEntryForFlowTest(User user, String sceneJson,String path,String sceneName,
												 ScriptHandler scriptHandler, Class handlerClass) throws CreateScriptException {
		try {
			// 根据Handler类型从Spring上下文中获取相应的Handler对象
			Object typedScriptHandler = config.getAppContext().getBean(handlerClass);
			// 为Handler对象设置场景所需的json字符串
			Method setSceneJson = handlerClass.getMethod("setSceneJson", String.class);
			setSceneJson.invoke(typedScriptHandler, sceneJson);
			prepareNewEntryByKey(user, path, sceneName, sceneName, "", (ScriptHandler) typedScriptHandler, false, null);
		}catch (Throwable throwable){
			throwable.printStackTrace();
			throw new CreateScriptException("Prepare project ERROE for scene "+sceneName+", transactional rollback !");
		}
	}
